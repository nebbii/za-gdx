...
import com.nebbii.zagdx.ImageLoader.NpcShurmakAnimationGroup;
import com.nebbii.zagdx.NpcShurmak;
import com.nebbii.zagdx.World;

// TODO: extractor reported 14 frames per group, but only 9 sprite files exist on disk per group;
// using the real counts.
public class NpcShurmakAnimation extends GameAnimation {
    private final NpcShurmak npc;
    ...
    public NpcShurmakAnimation(NpcShurmak npc) {
        super("idle0");
        this.npc = npc;
    ...
    private Animation<TextureRegion> initIdle0() {
        ...
        return initIdle(NpcShurmakAnimationGroup.IDLE0, frameData, idle0OffsetX, idle0OffsetY);
    }
    ...
    private Animation<TextureRegion> initIdle1() {
        ...
        return initIdle(NpcShurmakAnimationGroup.IDLE1, frameData, idle1OffsetX, idle1OffsetY);
    }

    private Animation<TextureRegion> initIdle(
        NpcShurmakAnimationGroup group,
        int[][] frameData,
        int[] animationOffsetX,
        int[] animationOffsetY
    ) {
        Texture[] textures = World.images.getNpcShurmakAnimation(group);
