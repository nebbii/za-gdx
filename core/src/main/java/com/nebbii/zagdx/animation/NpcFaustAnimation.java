...
import com.nebbii.zagdx.ImageLoader.NpcFaustAnimationGroup;
import com.nebbii.zagdx.NpcFaust;
import com.nebbii.zagdx.World;

// TODO: extractor reported 4 frames per group, but only 3 sprite files exist on disk per group;
// using the real counts. This is a stub, so only idle0 is ever played.
public class NpcFaustAnimation extends GameAnimation {
    private final NpcFaust npc;
    ...
    public NpcFaustAnimation(NpcFaust npc) {
        super("idle0");
        this.npc = npc;
    ...
    private Animation<TextureRegion> initIdle0() {
        ...
        return initIdle(NpcFaustAnimationGroup.IDLE0, frameData, idle0OffsetX, idle0OffsetY);
    }
    ...
    private Animation<TextureRegion> initIdle1() {
        ...
        return initIdle(NpcFaustAnimationGroup.IDLE1, frameData, idle1OffsetX, idle1OffsetY);
    }

    private Animation<TextureRegion> initIdle(
        NpcFaustAnimationGroup group,
        int[][] frameData,
        int[] animationOffsetX,
        int[] animationOffsetY
    ) {
        Texture[] textures = World.images.getNpcFaustAnimation(group);
