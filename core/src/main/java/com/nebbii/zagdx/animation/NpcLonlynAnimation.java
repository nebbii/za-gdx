...
import com.nebbii.zagdx.ImageLoader.NpcLonlynAnimationGroup;
import com.nebbii.zagdx.NpcLonlyn;
import com.nebbii.zagdx.World;

// TODO: extractor reported 8 frames per group, but only 5 sprite files exist on disk per group;
// using the real counts.
public class NpcLonlynAnimation extends GameAnimation {
    private final NpcLonlyn npc;
    ...
    public NpcLonlynAnimation(NpcLonlyn npc) {
        super("idle0");
        this.npc = npc;
    ...
    private Animation<TextureRegion> initIdle0() {
        ...
        return initIdle(NpcLonlynAnimationGroup.IDLE0, frameData, idle0OffsetX, idle0OffsetY);
    }
    ...
    private Animation<TextureRegion> initIdle1() {
        ...
        return initIdle(NpcLonlynAnimationGroup.IDLE1, frameData, idle1OffsetX, idle1OffsetY);
    }
    ...
    private Animation<TextureRegion> initIdle2() {
        ...
        return initIdle(NpcLonlynAnimationGroup.IDLE2, frameData, idle2OffsetX, idle2OffsetY);
    }

    private Animation<TextureRegion> initIdle(
        NpcLonlynAnimationGroup group,
        int[][] frameData,
        int[] animationOffsetX,
        int[] animationOffsetY
    ) {
        Texture[] textures = World.images.getNpcLonlynAnimation(group);
        ...
        Animation<TextureRegion> anim = new Animation<TextureRegion>(0.15f, frames);
