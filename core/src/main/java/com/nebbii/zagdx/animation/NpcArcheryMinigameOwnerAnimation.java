...
import com.nebbii.zagdx.ImageLoader.NpcArcheryMinigameOwnerAnimationGroup;
import com.nebbii.zagdx.NpcArcheryMinigameOwner;
import com.nebbii.zagdx.World;

// TODO: extractor reported 4 frames per group, but only 3 sprite files exist on disk per group;
// using the real counts.
public class NpcArcheryMinigameOwnerAnimation extends GameAnimation {
    private final NpcArcheryMinigameOwner npc;
    ...
    public NpcArcheryMinigameOwnerAnimation(NpcArcheryMinigameOwner npc) {
        super("idle0");
        this.npc = npc;
    ...
    private Animation<TextureRegion> initIdle0() {
        ...
        return initIdle(NpcArcheryMinigameOwnerAnimationGroup.IDLE0, frameData, idle0OffsetX, idle0OffsetY);
    }
    ...
    private Animation<TextureRegion> initIdle1() {
        ...
        return initIdle(NpcArcheryMinigameOwnerAnimationGroup.IDLE1, frameData, idle1OffsetX, idle1OffsetY);
    }
    ...
    private Animation<TextureRegion> initIdle2() {
        ...
        return initIdle(NpcArcheryMinigameOwnerAnimationGroup.IDLE2, frameData, idle2OffsetX, idle2OffsetY);
    }

    private Animation<TextureRegion> initIdle(
        NpcArcheryMinigameOwnerAnimationGroup group,
        int[][] frameData,
        int[] animationOffsetX,
        int[] animationOffsetY
    ) {
        Texture[] textures = World.images.getNpcArcheryMinigameOwnerAnimation(group);
