...
import com.nebbii.zagdx.ImageLoader.NpcGwynlaAnimationGroup;
import com.nebbii.zagdx.NpcGwynla;
import com.nebbii.zagdx.World;

// TODO: extractor reported 8/8/8/4 frames for up/right/down/left, but 5/5/5/4 sprite files exist
// on disk; using the real counts. Npc.getDirection() currently always returns DOWN (the base
// class does not track a real facing), so this always shows IDLEDOWN today.
public class NpcGwynlaAnimation extends GameAnimation {
    private final NpcGwynla npc;
    ...
    public NpcGwynlaAnimation(NpcGwynla npc) {
        super("idleDown");
        this.npc = npc;
    ...
    public TextureRegion playCurrentAnimation() {
        ...
        switch (npc.getDirection()) {
            ...
            case LEFT:
                ...
                break;
            default:
                throw new IllegalStateException("Unhandled NpcGwynla direction");
        }
    ...
    private Animation<TextureRegion> initIdleUp() {
        Texture[] textures = World.images.getNpcGwynlaAnimation(NpcGwynlaAnimationGroup.IDLEUP);
    ...
    private Animation<TextureRegion> initIdleRight() {
        Texture[] textures = World.images.getNpcGwynlaAnimation(NpcGwynlaAnimationGroup.IDLERIGHT);
    ...
    private Animation<TextureRegion> initIdleDown() {
        Texture[] textures = World.images.getNpcGwynlaAnimation(NpcGwynlaAnimationGroup.IDLEDOWN);
    ...
    private Animation<TextureRegion> initIdleLeft() {
        Texture[] textures = World.images.getNpcGwynlaAnimation(NpcGwynlaAnimationGroup.IDLELEFT);
